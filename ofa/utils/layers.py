...
__all__ = [
	...
	'ResidualBlock', 'ResNetBottleneckBlock','ResNetResidualBlock'
]
...
def set_layer_from_config(layer_config):
	...
	name2layer = {
		...
		ResNetResidualBlock.__name__: ResNetResidualBlock,
	}
...
class ResNetResidualBlock(MyModule):
	...
	def __init__(self, in_channels, out_channels,
		...
		pad = get_same_padding(self.kernel_size)
		self.conv1 = nn.Sequential(OrderedDict([
			('conv', nn.Conv2d(self.in_channels, feature_dim, kernel_size, stride, pad, bias=False)),
			('bn', nn.BatchNorm2d(feature_dim)),
			('act', build_activation(self.act_func, inplace=True)),
		]))
		...
		self.conv2 = nn.Sequential(OrderedDict([
			('conv', nn.Conv2d(feature_dim, self.out_channels, kernel_size, 1, pad, bias=False)),
			('bn', nn.BatchNorm2d(self.out_channels)),
		]))
	...
	def module_str(self):
		return '(%s, %s)' % (
			'%dx%d_ResidualConv_%d->%d->%d_S%d_G%d' % (
				self.kernel_size, self.kernel_size, self.in_channels, self.mid_channels, self.out_channels,
				self.stride, self.groups
			),
			'Identity' if isinstance(self.downsample, IdentityLayer) else self.downsample_mode,
	...
	@staticmethod
	def build_from_config(config):
		return ResNetResidualBlock(**config)