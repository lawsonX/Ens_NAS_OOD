...
__all__ = ['ResNets', 'ResNet50', 'ResNet50D','ResNetEns']


class ResNets(MyNetwork):

	BASE_DEPTH_LIST = [2, 2, 2, 2]
	STAGE_WIDTH_LIST = [32,64,128,256] #[128, 256, 512, 1024] #[256, 512, 1024, 2048]
...
class ResNet18(ResNets):
	def __init__(self, n_classes=10, width_mult=1.0, bn_param=(0.1, 1e-5), dropout_rate=0,
		...
		for d, width, s in zip(depth_list, stage_width_list, stride_list):
			for i in range(d):
				...
				bottleneck_block = ResidualBlock(
