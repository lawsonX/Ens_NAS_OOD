...
class OFAResNets(ResNets):
	def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0,
	             depth_list=2, expand_ratio_list=0.25, width_mult_list=1.0,outputs=1):
		...
		# classifier
		if outputs > 1 :
			classifier = DynamicMultiHeadLinearLayer(input_channel, n_classes, outputs)
		else:
	...
	def get_active_subnet(self, preserve_weight=True):
		...
		classifier = self.classifier.get_active_subnet(input_channel, preserve_weight=False)
...
class OFAResNets18(ResNets):
	def __init__(self, n_classes=10, bn_param=(0.1, 1e-5), dropout_rate=0,
	             depth_list=2, expand_ratio_list=1, width_mult_list=1.0,outputs=1):
		...
		input_channel = [
			make_divisible(16 * width_mult, MyNetwork.CHANNEL_DIVISIBLE) for width_mult in self.width_mult_list
		]
		mid_input_channel = [
			make_divisible(channel // 2, MyNetwork.CHANNEL_DIVISIBLE) for channel in input_channel
		]
		...
		# classifier
		if outputs > 1 :
			classifier = DynamicMultiHeadLinearLayer(input_channel, n_classes, outputs,dropout_rate=dropout_rate)
		else:
			classifier = DynamicLinearLayer(input_channel, n_classes, dropout_rate=dropout_rate)
	...
	def set_activate_net_ensemble(self, )
